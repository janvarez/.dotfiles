...
--convert to table
--[[
        print("bartab:", bartab)  -- Add this line for debugging
        -- Check if bartab is not nil and not an empty string
        if bartab and bartab ~= "" then
            local bartab_function, load_error = load("return " .. bartab)
            
            -- Check if loading the function was successful
            if bartab_function then
                local bartab_values = bartab_function()
                
                -- Check if bartab_values is not nil
                if bartab_values then
                    local bar_startx = bartab_values[1]
                    local bar_starty = bartab_values[2]
                    -- Continue with the rest of your code using bartab_values
                    -- ...
                else
                    print("Error evaluating bartab function")
                end
            else
                print("Error loading bartab function:", load_error)
            end
        else
            print("Error: bartab is nil or empty")
        end
...
--local bartab = {6, 55, conky_parse("${memperc}"), 100, 97, 2, 10, 1, 0xFFFFFF, 0.25, 0x00FF00, 1, 0xFFFF00, 1, 0xFF0000, 1}
--local bartab = {6, 115, conky_parse("${swapperc}"), 100, 97, 2, 10, 1, 0xFFFFFF, 0.25, 0x00FF00, 1, 0xFFFF00, 1, 0xFF0000, 1}
