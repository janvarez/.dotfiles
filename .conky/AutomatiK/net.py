...
# iwgetid wlx90de80821a6a -r  = wifisalon

List = []

for Interface in NumberOfInterfaces:
	TestInterface="""${if_match "${addr """+str(Interface)+"""}"!="No Address"}"""	
	ShowInterface="${goto 14}"+NetworkInterface[Language]+" ${color3}"+str(Interface)+"${color}${goto 160}"+"\n"
	ShowWirelessInfo="""${if_match "${wireless_mode """+str(Interface)+"""}"!=""}${goto 14}${font WIFI}b${font}ESSID : ${wireless_essid """+str(Interface)+"""}  ${offset 10}(${wireless_link_qual_perc """+str(Interface)+"""} %)    ${wireless_bitrate """+str(Interface)+"""} ${endif} """+"\n"
	UpDown="${goto 14}${color1}"+Sending[Language]+" ${color3}${upspeedf "+str(Interface)+"}${color1}${alignr}${goto 180}${color1}"+Receiving[Language]+"${color1} ${color3}${downspeedf "+str(Interface)+"}${color}"+"\n"
...
Local_IP_Value="       "+LocaleIPadress[Language]+"${addr "+str(Interface)+"}"+"\n"

# print TotalNetwork
total=header+txt01+Public_IP_Value+Local_IP_Value+TotalNetwork + "]];"
#print total
